...
public class BitSequence implements Iterable<Integer>, Serializable {
   ...
   */
  public void setBit(long index, int bit) {
    if (index < 0 || index >= numBits) {
      throw new IndexOutOfBoundsException();
    }

    int whichByte = (int) (index / 8);
  ...
  public boolean equals(Object other) {
    ...
    // index.
    for (int i = 0; i < Math.ceil(length() / 8.0); i++) {
   ...
   */
  private void trim() {
    byte[] newBytes = new byte[(int) ((numBits + 7) / 8)];
